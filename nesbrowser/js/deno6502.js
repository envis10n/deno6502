...
class CPU {
    ...
    load(program) {
        for(let i = 0; i < program.length; i++){
            this.write(0x0600 + i, program[i]);
        }
        this.write_u16(0xfffc, 0x0600);
    }
    reset() {
        this.register_a = 0;
        this.register_x = 0;
...
function readScreenState(cpu, frame) {
    let frame_idx = 0;
    let update = false;
    for (let i = 0x0200; i < 0x600; i++) {
        const color_idx = cpu.read(i);
        const [r, g, b] = pixelColor(color_idx);
        if (frame[frame_idx] != r || frame[frame_idx + 1] != g || frame[frame_idx + 2] != b) {
            frame[frame_idx] = r;
            frame[frame_idx + 1] = g;
            frame[frame_idx + 2] = b;
            frame[frame_idx + 3] = 0xff;
            update = true;
        }
        frame_idx += 4;
    }
...
function onStartPressed() {
    const screen = document.getElementById("nes_screen");
    if (screen == null) throw new Error("NES Screen Canvas element is not accessible.");
    let screen_state = new Uint8Array(32 * 4 * 32);
    const game_code = new Uint8Array([
    ...
    is_started = true;
    cpu.interpret(game_code, (cpu) => {
        handleInput(cpu);
        cpu.write(0xfe, randInRange(16, 1));
        if (readScreenState(cpu, screen_state)) {
            // Update Canvas
